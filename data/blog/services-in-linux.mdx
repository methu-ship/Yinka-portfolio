...
To create a service, a systemd service is configured using a systemd unit file. The file should be named after the service you want to create then save it in the `/usr/lib/systemd/system` directory with a `.service` extension.
...
WantedBy=multi-user.target \#service will root at boot after this service starts
```    
