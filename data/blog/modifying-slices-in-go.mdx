...
Pointers are variables that store the memory address of another variable. They enable you to reference and manipulate the value stored at that address directly, allowing for more efficient memory usage and the ability to modify variables and data structures indirectly.

> The term "indirectly" emphasizes that the modification is done through a reference to the variable's memory address rather than by the variable's name directly. 
...
Slices, Maps, and Channels in Go all behave like pointers by default.
