...
- **Nodes**: A Node is a worker machine in Kubernetes, previously known as a 'minion'. A node may be a VM or physical machine, depending on the cluster. Each Node contains the services necessary to run Pods and is managed by the master components. Think of it as a big playground where many of these kids can play and work together.
...
Think of Kubernetes networking like transitioning from a city with traditional roads and traffic (physical networks) to a futuristic city where cars (data packets) teleport from one place to another (virtual networks).

### Key Rules of the Kubernetes Network Model
- Individual Pod IPs: Each Pod gets its own unique IP address, making inter-Pod communication straightforward without port mapping.
...
- Kubernetes has it's in-built CNI called the Kubernetes CNI
...
### eBPF
eBPF is a technology used to safely and efficiently extend the capabilities of the kernel without requiring to change kernel source code or load kernel modules. eBPF enables the development of powerful new tools for the Cloud Native ecosystem that offer enhanced observability, efficient networking and improved performance management, revolutionizing the way cloud-native applications are built, run and operated.
...
- **Policy Enforcement**: Cilium enforces network policies defined in Kubernetes at both Layer 3/4 (IP/Port level) and Layer 7 (application level), allowing for more granular control compared to standard Kubernetes Network Policies.
...
### What does this do?
**Target Pods**:
  - This network policy only applies to pods with the label `role: db`.
...
**Ingress Rules (for traffic entering the pods)**:
  ...
  - Allows traffic from any pods in namespaces labeled `project: myproject`.
  - Allows traffic from pods labeled `role: frontend`.
...
### CiliumNetworkPolicy
According to Cilium's official documentatiin, the `CiliumNetworkPolicy` is very similar to the standard NetworkPolicy. The purpose is to provide the functionality which is not yet supported in NetworkPolicy. Cilium supports specification of policies at Layers 3-7 for both ingress and egress.
...
- **IP/CIDR based**: used to define policies to and from endpoints which are not managed by Cilium and thus do not have labels associated with them.
- **DNS based**: selects remote, non-cluster, peers using DNS names converted to IPs via DNS lookups.

Based on all we know about Cilium network policies, let's replicate the Kubernetes network policy example.
...
Targets Pods with role: db Label: The policy applies to pods labeled as `role: db`.
...
So Kubernetes network policies and cilium network policies are quite similar but Cilium gives you much more power to define your cluster security at both Layer 3/4 and Layer 7 of the [OSI model](https://en.wikipedia.org/wiki/OSI_model).

## Learning Resources
If you're a curious to learn more about Cilium especially at Scale, you should check out the project's [case studies page](https://www.cncf.io/case-studies/?_sft_lf-project=cilium) on the CNCF wesbite. Also, here are some useful learning resources to get you started with Cilium:
